import JackettLogo from '@app/assets/services/jackett.svg';
...
import IndexerModal from '@app/components/Settings/IndexerModal';
...
import WhisparrModal from '@app/components/Settings/WhisparrModal';
...
import type { IndexerSettings, RadarrSettings, SonarrSettings, WhisparrSettings } from '@server/lib/settings';
  ...
  return (
    <li className="col-span-1 rounded-lg bg-gray-800 shadow ring-1 ring-gray-500">
      <div className="flex w-full items-center justify-between space-x-6 p-6">
        ...
        >
          {isJackett ? (
            <JackettLogo className="h-10 w-10 flex-shrink-0" />
          ) : isProwlarr ? (
            <ProwlarrLogo className="h-10 w-10 flex-shrink-0" />
          ) : isWhisparr ? (
      ...
      <div className="border-t border-gray-500">
        <div className="-mt-px flex">
          <div className="flex w-0 flex-1 border-r border-gray-500">
            <button
              onClick={() => onEdit()}
              className="focus:ring-[#ff3366] relative -mr-px inline-flex w-0 flex-1 items-center justify-center rounded-bl-lg border border-transparent py-4 text-sm font-medium leading-5 text-gray-200 transition duration-150 ease-in-out hover:text-white focus:z-10 focus:border-gray-500 focus:outline-none"
            >
          ...
          <div className="-ml-px flex w-0 flex-1">
            <button
              onClick={() => onDelete()}
              className="focus:ring-[#ff3366] relative inline-flex w-0 flex-1 items-center justify-center rounded-br-lg border border-transparent py-4 text-sm font-medium leading-5 text-gray-200 transition duration-150 ease-in-out hover:text-white focus:z-10 focus:border-gray-500 focus:outline-none"
            >
...
const SettingsServices = () => {
  ...
  const {
    data: whisparrData,
    error: whisparrError,
    mutate: revalidateWhisparr,
  } = useSWR<WhisparrSettings[]>('/api/v1/settings/whisparr');
  const {
    data: indexerData,
    error: indexerError,
    mutate: revalidateIndexer,
  } = useSWR<IndexerSettings[]>('/api/v1/settings/indexer');
  ...
  const [editWhisparrModal, setEditWhisparrModal] = useState<{
    open: boolean;
  ...
  const [editIndexerModal, setEditIndexerModal] = useState<{
    open: boolean;
  ...
  const [deleteServerModal, setDeleteServerModal] = useState<{
    open: boolean;
    type: 'radarr' | 'sonarr' | 'whisparr' | 'indexer';
  ...
  return (
    <>
      ...
      {editWhisparrModal.open && (
        <WhisparrModal
          whisparr={editWhisparrModal.whisparr}
          onClose={() => {
            setEditWhisparrModal({ open: false, whisparr: null });
          }}
          onSave={() => {
            revalidateWhisparr();
            mutate('/api/v1/settings/public');
      ...
      {editIndexerModal.open && (
        <IndexerModal
          indexer={editIndexerModal.indexer}
          onClose={() => {
            setEditIndexerModal({ open: false, indexer: null });
          }}
          onSave={() => {
            revalidateIndexer();
            mutate('/api/v1/settings/public');
            ...
            serverType:
              deleteServerModal.type === 'radarr'
                ...
                : deleteServerModal.type === 'sonarr'
                  ? 'Sonarr'
                  : deleteServerModal.type === 'whisparr'
                    ? 'Whisparr'
                    : 'Indexer',
          })}
        >
      ...
      <div className="section">
        {!whisparrData && !whisparrError && <LoadingSpinner />}
        {whisparrData && !whisparrError && (
          <>
            {whisparrData.length > 0 &&
              (!whisparrData.some((whisparr) => whisparr.isDefault) ? (
                <Alert
                  title={intl.formatMessage(messages.noDefaultServer, {
                    serverType: 'Whisparr',
                    mediaType: intl.formatMessage(messages.mediaTypeScene),
                  })}
                />
              ) : !whisparrData.some(
                  (whisparr) => whisparr.isDefault && !whisparr.is4k
                ) ? (
                <Alert
                  title={intl.formatMessage(messages.noDefaultNon4kServer, {
                    serverType: 'Whisparr',
                ...
                whisparrData.some((whisparr) => whisparr.is4k) &&
                !whisparrData.some(
                  (whisparr) => whisparr.isDefault && whisparr.is4k
                ) && (
                  <Alert
            ...
            <ul className="grid max-w-6xl grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
              {whisparrData.map((whisparr) => (
                <ServerInstance
                  ...
                  onEdit={() =>
                    setEditWhisparrModal({ open: true, whisparr })
                  }
                  onDelete={() =>
                    setDeleteServerModal({
                      open: true,
              ...
              <li className="col-span-1 h-32 rounded-lg border-2 border-dashed border-gray-400 shadow sm:h-44">
                <div className="flex h-full w-full items-center justify-center">
                  <Button
                    ...
                    className="mb-3 mt-3"
                    onClick={() =>
                      setEditWhisparrModal({ open: true, whisparr: null })
                    }
                  >
                    <PlusIcon />
      ...
      <div className="mb-6 mt-10">
        <h3 className="heading">
          {intl.formatMessage(messages.indexersettings)}
        </h3>
        <p className="description">
          {intl.formatMessage(messages.serviceSettingsDescription, {
            serverType: 'Indexer',
      ...
      <div className="section">
        {!indexerData && !indexerError && <LoadingSpinner />}
        {indexerData && !indexerError && (
          <>
            <ul className="grid max-w-6xl grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
              {indexerData.map((idx) => (
                <ServerInstance
                  key={`indexer-config-${idx.id}`}
                  name={idx.name}
                  hostname={idx.hostname}
                  port={idx.port}
                  profileName=""
                  isSSL={idx.useSsl}
                  isJackett={idx.type === 'jackett'}
                  isProwlarr={idx.type === 'prowlarr'}
                  externalUrl={idx.externalUrl}
                  subtitle={`${intl.formatMessage(messages.indexerType)}: ${idx.type === 'jackett' ? 'Jackett' : 'Prowlarr'} · ${intl.formatMessage(messages.indexerPriority)}: ${idx.priority} · ${intl.formatMessage(messages.indexerEnabled)}: ${idx.enabled ? 'Yes' : 'No'}`}
                  onEdit={() =>
                    setEditIndexerModal({ open: true, indexer: idx })
                  }
                  onDelete={() =>
                    setDeleteServerModal({
                      open: true,
              ...
              <li className="col-span-1 h-32 rounded-lg border-2 border-dashed border-gray-400 shadow sm:h-44">
                <div className="flex h-full w-full items-center justify-center">
                  <Button
                    ...
                    className="mb-3 mt-3"
                    onClick={() =>
                      setEditIndexerModal({ open: true, indexer: null })
                    }
                  >
                    <PlusIcon />
                    <span>{intl.formatMessage(messages.addindexer)}</span>
                  </Button>
                </div>
              </li>
            </ul>
