...
const messages = defineMessages('components.Discover', {
  ...
  adulttrending: 'Trending Now 🔥',
  adultnewreleases: 'New Releases',
  adultpopularperformers: 'Popular Performers',
  adulttopstudios: 'Top Studios',
  adultcategory: 'Browse by Category',
    ...
    <>
      ...
      <div className="mb-4 flex items-center gap-3">
        <Tooltip content={intl.formatMessage(messages.filtertoggle)}>
          <Button
            buttonType={showFilterSidebar ? 'primary' : 'default'}
            className="flex items-center gap-2"
            onClick={() => setShowFilterSidebar(!showFilterSidebar)}
          >
            <FunnelIcon className="h-4 w-4" />
            <span className="hidden sm:inline">{intl.formatMessage(messages.filtertoggle)}</span>
          </Button>
        </Tooltip>
        {selectedCategories.length > 0 && (
          <span className="rounded-full px-2 py-0.5 text-xs font-medium" style={{ backgroundColor: 'rgba(255, 51, 102, 0.15)', color: '#ff6690' }}>
      ...
      {(isEditing ? sliders : discoverData)?.map((slider, index) => {
        ...
        switch (slider.type) {
          ...
          case DiscoverSliderType.TMDB_SEARCH:
            ...
            break;
          case DiscoverSliderType.ADULT_TRENDING:
            sliderComponent = (
              <MediaSlider
                sliderKey="adult-trending"
                title={intl.formatMessage(messages.adulttrending)}
                url="/api/v1/discover/adult/trending"
                linkUrl="/discover/adult/trending"
              />
            );
            break;
          case DiscoverSliderType.ADULT_NEW_RELEASES:
            sliderComponent = (
              <MediaSlider
                sliderKey="adult-new-releases"
                title={intl.formatMessage(messages.adultnewreleases)}
                url="/api/v1/discover/adult/new"
                linkUrl="/discover/adult/new"
              />
            );
            break;
          case DiscoverSliderType.ADULT_POPULAR_PERFORMERS:
            sliderComponent = (
              <MediaSlider
                sliderKey="adult-performers"
                title={intl.formatMessage(messages.adultpopularperformers)}
                url="/api/v1/discover/adult/performers"
              />
            );
            break;
          case DiscoverSliderType.ADULT_TOP_STUDIOS:
            sliderComponent = (
              <MediaSlider
                sliderKey="adult-studios"
                title={intl.formatMessage(messages.adulttopstudios)}
                url="/api/v1/discover/adult/studios"
              />
            );
            break;
          case DiscoverSliderType.ADULT_CATEGORY:
            sliderComponent = (
              <MediaSlider
                sliderKey="adult-category"
                title={intl.formatMessage(messages.adultcategory)}
                url="/api/v1/discover/adult/category"
              />
            );
            break;
