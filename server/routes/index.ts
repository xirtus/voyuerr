...
import adultMetadataRoutes from './adultMetadata';
...
router.get<unknown, StatusResponse>('/status', async (req, res) => {
  ...
  if (currentVersion.startsWith('develop-') && commitTag !== 'local') {
    const commits = await githubApi.getVoyeurrCommits();
    ...
    const releases = await githubApi.getVoyeurrReleases();
...
router.use('/scene', isAuthenticated(), sceneRoutes);
router.use('/performer', isAuthenticated(), performerRoutes);
router.use('/studio', isAuthenticated(), studioRoutes);
router.use('/adult-metadata', isAuthenticated(), adultMetadataRoutes);
router.use('/apikey', isAuthenticated(), apikeyRoutes);
router.use('/webhooks', isAuthenticated(), webhookRoutes);
router.use('/', isAuthenticated(), advancedRoutes);
