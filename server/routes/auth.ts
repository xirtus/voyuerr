    ...
    if (!user && !(await userRepository.count())) {
      ...
      if (
        ...
        if (user) {
          ...
          logger.info(
            'Sign-in attempt from Plex user with access to the media server; creating new Voyeurr user',
            {
              label: 'API',
              ip: req.ip,
              email: account.email,
...
authRoutes.post('/jellyfin', async (req, res, next) => {
  ...
  try {
    ...
    let deviceId = 'BOT_voyeurr';
    if (user && user.id === 1) {
      // Admin is always BOT_voyeurr
      deviceId = 'BOT_voyeurr';
    } else if (user && user.jellyfinDeviceId) {
      deviceId = user.jellyfinDeviceId;
    } else if (body.username) {
      deviceId = Buffer.from(`BOT_voyeurr_${body.username}`).toString('base64');
    }
    ...
    if (
      ...
      if (missingAdminUser) {
        logger.info(
          'Sign-in attempt from Jellyfin user with access to the media server; creating initial admin user for Voyeurr',
          {
            label: 'API',
            ip: req.ip,
        ...
        logger.info(
          'Sign-in attempt from Jellyfin user with access to the media server; editing admin user for Voyeurr',
          {
            label: 'API',
            ip: req.ip,
            jellyfinUsername: account.User.Name,
      ...
      const apiKey = await jellyfinClient.createApiToken('Voyeurr');
    ...
    // User already exists, let's update their information
      ...
      logger.info(
        'Sign-in attempt from Jellyfin user with access to the media server; creating new Voyeurr user',
        {
          label: 'API',
          ip: req.ip,
          jellyfinUsername: account.User.Name,
...
authRoutes.post('/local', async (req, res, next) => {
  ...
  try {
    ...
    if (!user || !(await user.passwordMatch(body.password))) {
      logger.warn('Failed sign-in attempt using invalid Voyeurr password', {
        label: 'API',
        ip: req.ip,
        email: body.email,
        userId: user?.id,
    ...
    logger.error('Something went wrong authenticating with Voyeurr password', {
      label: 'API',
      errorMessage: e.message,
      ip: req.ip,
      email: body.email,
        ...
        try {
          ...
          try {
            await axios.delete(`${baseUrl}/Devices`, {
              ...
              headers: {
                'X-Emby-Authorization': `MediaBrowser Client="Voyeurr", Device="Voyeurr", DeviceId="voyeurr", Version="${
                  settings.main.mediaServerType === MediaServerType.EMBY
                    ? '1.0.0'
                    : getAppVersion()
