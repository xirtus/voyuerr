  ...
  async (req, res) => {
    ...
    if (await userRepository.exist({ where: { plexId: account.id } })) {
      return res.status(422).json({
        message: 'This Plex account is already linked to a Voyeurr user',
      });
    }
...
userSettingsRoutes.post<{ username: string; password: string }>(
  ...
  async (req, res) => {
    ...
    if (
      ...
      return res.status(422).json({
        message: 'The specified account is already linked to a Voyeurr user',
      });
    }

    const hostname = getHostname();
    const deviceId = Buffer.from(
      req.user?.id === 1 ? 'BOT_voyeurr' : `BOT_voyeurr_${req.user.username ?? ''}`
    ).toString('base64');
