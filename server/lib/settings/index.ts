...
export interface WhisparrSettings extends DVRSettings {
  minimumAvailability: string;
  /** Whether to search for scenes immediately upon adding them */
  searchOnAdd: boolean;
  /** Whether to automatically monitor performers added to Whisparr */
  monitorPerformers: boolean;
  /** Whether to automatically monitor studios added to Whisparr */
  monitorStudios: boolean;
}

/** Type of indexer service. */
export type IndexerServiceType = 'jackett' | 'prowlarr';

/** Settings for a connected indexer server (Jackett or Prowlarr). */
export interface IndexerSettings {
  ...
  priority: number;
  /** Category IDs to include when searching (Newznab format). */
  activeCategories: number[];
  /** Whether this indexer server is enabled for searches. */
  enabled: boolean;
  /** Whether to include this server's results. */
  syncEnabled: boolean;
  /** Cached list of indexer names/IDs from the server. */
...
export interface StashSettings {
  ...
  useSsl: boolean;
  apiKey?: string;
  /** Sync frequency in cron expression. Default: daily at 3am */
  syncSchedule?: string;
  /** Last sync timestamp. */
  lastSync?: number;
}

export interface AdultMetadataProviderConfig {
  enabled: boolean;
  apiKey?: string;
  /** Priority weight for conflict resolution (0–100). */
  priority: number;
}

export interface AdultMetadataSettings {
  /** ThePornDB configuration. */
  tpdb: AdultMetadataProviderConfig;
  /** R18.dev / JavDB configuration. */
  r18: AdultMetadataProviderConfig;
  /** AdultDVDEmpire configuration. */
  adultdvdempire: AdultMetadataProviderConfig;
  /** nHentai configuration. */
  nhentai: AdultMetadataProviderConfig;
  /** Hanime configuration. */
  hanime: AdultMetadataProviderConfig;
  /** Fakku configuration. */
  fakku: AdultMetadataProviderConfig;
  /** Auto-refresh interval in cron expression. */
...
export interface AllSettings {
  ...
  whisparr: WhisparrSettings[];
  indexer: IndexerSettings[];
  stash: StashSettings;
  adultMetadata: AdultMetadataSettings;
...
class Settings {
  ...
  constructor(initialSettings?: AllSettings) {
    this.data = {
      ...
      adultMetadata: {
        tpdb: { enabled: false, apiKey: '', priority: 90 },
        r18: { enabled: false, apiKey: '', priority: 85 },
        adultdvdempire: { enabled: false, apiKey: '', priority: 75 },
        nhentai: { enabled: false, apiKey: '', priority: 70 },
        hanime: { enabled: false, apiKey: '', priority: 60 },
        fakku: { enabled: false, apiKey: '', priority: 65 },
        refreshSchedule: '0 2 * * *',
      },
      public: {
        initialized: false,
  ...
  get whisparr(): WhisparrSettings[] {
    return this.data.whisparr;
  }

  set whisparr(data: WhisparrSettings[]) {
    this.data.whisparr = data;
  }

  get indexer(): IndexerSettings[] {
    return this.data.indexer;
  }

  set indexer(data: IndexerSettings[]) {
    this.data.indexer = data;
  }
  ...
  get fullPublicSettings(): FullPublicSettings {
    return {
      ...
      scene4kEnabled: this.data.whisparr.some(
        (whisparr) => whisparr.is4k && whisparr.isDefault
      ),
  ...
  get stash(): StashSettings {
    return this.data.stash;
  }

  set stash(data: StashSettings) {
    this.data.stash = mergeSettings(this.data.stash, data);
  }

  get adultMetadata(): AdultMetadataSettings {
    return this.data.adultMetadata;
  }

  set adultMetadata(data: AdultMetadataSettings) {
    this.data.adultMetadata = mergeSettings(this.data.adultMetadata, data);
  }
