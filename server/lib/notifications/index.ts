...
class NotificationManager {
  ...
  /**
   * Sanitize notification subject for privacy.
   * If the requesting user has notificationPrivacy enabled, replace
   * scene/media titles with generic text.
   */
  private sanitizePayload(payload: NotificationPayload): NotificationPayload {
    const notifyUser = payload.notifyUser;

    // Check if the requesting user has notification privacy enabled
    const hasPrivacy = (notifyUser?.settings as any)?.notificationPrivacy === true;

    if (hasPrivacy && payload.subject) {
      const sanitized = { ...payload };

      // Replace title with generic media type reference
      if (payload.media?.mediaType) {
        sanitized.subject = `New adult content available`;
      } else {
        sanitized.subject = 'Voyeurr notification';
      }

      // Also sanitize the message body if it contains a title
      if (sanitized.message && payload.media?.title) {
        sanitized.message = sanitized.message.replace(
          new RegExp(payload.media.title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi'),
          '[content]'
        );
      }
  ...
  public sendNotification(
    ...
    // Phase 6 privacy: sanitize titles for users with notification privacy enabled
