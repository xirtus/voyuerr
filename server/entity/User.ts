...
export class User {
  ...
  @Column({ nullable: true })
  public sceneQuotaLimit?: number;

  @Column({ nullable: true })
  public sceneQuotaDays?: number;

  @Column({ nullable: true })
  public collectionQuotaLimit?: number;

  @Column({ nullable: true })
  public collectionQuotaDays?: number;
  ...
  public async getQuota(): Promise<QuotaResponse> {
    ...
    // Default quota values from settings (with scene/collection fallback to movie/tv defaults)
    const defaultSceneQuota =
      (defaultQuotas as Record<string, { quotaLimit?: number; quotaDays?: number }>)
        ?.scene ?? defaultQuotas.movie;
    const defaultCollectionQuota =
      (defaultQuotas as Record<string, { quotaLimit?: number; quotaDays?: number }>)
        ?.collection ?? defaultQuotas.tv;
    ...
    // Scene quota (new adult content model)
    const sceneQuotaLimit = !canBypass
      ? (this.sceneQuotaLimit ?? defaultSceneQuota.quotaLimit)
      : 0;
    const sceneQuotaDays =
      this.sceneQuotaDays ?? defaultSceneQuota.quotaDays;

    const sceneDate = new Date();
    if (sceneQuotaDays) {
      sceneDate.setDate(sceneDate.getDate() - sceneQuotaDays);
    }

    const sceneQuotaUsed = sceneQuotaLimit
      ? await requestRepository.count({
          where: {
            requestedBy: { id: this.id },
            ...(sceneQuotaDays ? { createdAt: AfterDate(sceneDate) } : {}),
            type: MediaType.MOVIE,
            status: Not(MediaRequestStatus.DECLINED),
            is4k: false,
          },
    ...
    // Collection quota (new adult content model)
    const collectionQuotaLimit = !canBypass
      ? (this.collectionQuotaLimit ?? defaultCollectionQuota.quotaLimit)
      : 0;
    const collectionQuotaDays =
      this.collectionQuotaDays ?? defaultCollectionQuota.quotaDays;

    const collectionDate = new Date();
    if (collectionQuotaDays) {
      collectionDate.setDate(collectionDate.getDate() - collectionQuotaDays);
    }
    const collectionQuotaStartDate = collectionDate.toJSON();
    const collectionQuotaUsedQuery = requestRepository
      .createQueryBuilder('request')
      .leftJoin('request.requestedBy', 'requestedBy')
      .where('request.type = :requestType', {
        requestType: MediaType.TV,
    ...
    if (collectionQuotaDays) {
      collectionQuotaUsedQuery.andWhere('request.createdAt > :date', {
        date: collectionQuotaStartDate,
      });
    }

    const collectionQuotaUsed = collectionQuotaLimit
      ? await collectionQuotaUsedQuery.getCount()
      : 0;
    ...
    return {
      ...
      scene: {
        days: sceneQuotaDays,
        limit: sceneQuotaLimit,
        used: sceneQuotaUsed,
        remaining: sceneQuotaLimit
          ? Math.max(0, sceneQuotaLimit - sceneQuotaUsed)
          : undefined,
        restricted: !!(
          sceneQuotaLimit && sceneQuotaLimit - sceneQuotaUsed <= 0
        ),
      },
      collection: {
        days: collectionQuotaDays,
        limit: collectionQuotaLimit,
        used: collectionQuotaUsed,
        remaining: collectionQuotaLimit
          ? Math.max(0, collectionQuotaLimit - collectionQuotaUsed)
          : undefined,
        restricted: !!(
          collectionQuotaLimit && collectionQuotaLimit - collectionQuotaUsed <= 0
        ),
      },
    };
  }
