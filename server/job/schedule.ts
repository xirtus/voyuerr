...
import { whisparrScanner } from '@server/lib/scanners/whisparr';
import stashSync from '@server/lib/scanners/stash';
import adultScanner from '@server/lib/scanners/adult';
import { BackupManager } from '@server/lib/backupManager';
...
export const startJobs = (): void => {
  ...
  // Run full whisparr scan every 24 hours
  scheduledJobs.push({
    id: 'whisparr-scan',
    name: 'Whisparr Scan',
    type: 'process',
    interval: 'hours',
    cronSchedule: jobs['whisparr-scan'].schedule,
    job: schedule.scheduleJob(jobs['whisparr-scan'].schedule, () => {
      logger.info('Starting scheduled job: Whisparr Scan', { label: 'Jobs' });
      whisparrScanner.run();
    }),
    running: () => whisparrScanner.status().running,
    cancelFn: () => whisparrScanner.cancel(),
  });

  // Stash metadata sync — imports performers/studios/scenes from Stash
  scheduledJobs.push({
    id: 'stash-sync',
    name: 'Stash Metadata Sync',
    type: 'process',
    interval: 'hours',
    cronSchedule: jobs['stash-sync'].schedule,
    job: schedule.scheduleJob(jobs['stash-sync'].schedule, () => {
      logger.info('Starting scheduled job: Stash Metadata Sync', { label: 'Jobs' });
      stashSync.run().catch((e) => {
        logger.error('Stash sync job failed', {
          label: 'Jobs',
          errorMessage: e.message,
        });
      });
    }),
    running: () => stashSync.running,
  });
  ...
  scheduledJobs.push({
    ...
    job: schedule.scheduleJob(jobs['image-cache-cleanup'].schedule, () => {
      ...
      // Clean adult metadata image caches
      ImageProxy.clearCache('tpdb');
      ImageProxy.clearCache('r18');
  ...
  // Adult metadata refresh — runs daily at 2am by default
  scheduledJobs.push({
    id: 'adult-metadata-refresh',
    name: 'Adult Metadata Refresh',
    type: 'process',
    interval: 'hours',
    cronSchedule: jobs['adult-metadata-refresh'].schedule,
    job: schedule.scheduleJob(jobs['adult-metadata-refresh'].schedule, () => {
      logger.info('Starting scheduled job: Adult Metadata Refresh', {
        label: 'Jobs',
      });
      adultScanner.run().catch((e) => {
        logger.error('Adult metadata refresh job failed', {
          label: 'Jobs',
          errorMessage: e.message,
        });
      });
    }),
    running: () => adultScanner.status.running,
    cancelFn: () => adultScanner.cancel(),
  });
  ...
  scheduledJobs.push({
    ...
    type: 'process',
    interval: 'hours',
    cronSchedule: jobs['backup-database'].schedule,
    job: schedule.scheduleJob(jobs['backup-database'].schedule, () => {
      logger.info('Starting scheduled job: Database Backup', { label: 'Jobs' });
      BackupManager.scheduledBackup().catch((e) => {
        logger.error('Database backup failed', { label: 'Jobs', errorMessage: e.message });
      });
    }),
  });
