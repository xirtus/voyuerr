...
import checkVoyeurrMerge from '@server/lib/voyeurrMerge';
import { getSettings } from '@server/lib/settings';
import logger from '@server/logger';
import clearCookies from '@server/middleware/clearcookies';
import routes from '@server/routes';
import healthRoutes, { gracefulShutdown, isServerShuttingDown } from '@server/routes/health';
import avatarproxy from '@server/routes/avatarproxy';
import imageproxy from '@server/routes/imageproxy';
import { appDataPermissions } from '@server/utils/appDataVolume';
import { getAppVersion } from '@server/utils/appVersion';
import createCustomProxyAgent from '@server/utils/customProxyAgent';
import { initEncryption } from '@server/utils/encryption';
import { logProxyStatus } from '@server/utils/vpnProxy';
...
const API_SPEC_PATH = path.join(__dirname, '../voyeurr-api.yml');

logger.info(`Starting Voyeurr version ${getAppVersion()}`);
...
app
  .prepare()
  .then(async () => {
    // Run Voyeurr to Voyeurr migration
    await checkVoyeurrMerge();
    ...
    // Register HTTP/SOCKS5 proxy (VPN-aware)
    if (settings.network.proxy.enabled) {
      await createCustomProxyAgent(
        settings.network.proxy,
        settings.network.forceIpv4First
      );
      logProxyStatus(settings.network.proxy);
    }

    // Initialize field-level encryption for sensitive DB data
    initEncryption(process.env.VOYEURR_ENCRYPTION_KEY);

    // Phase 6: Zero telemetry / analytics / crash reporting
    // next telemetry disable is handled in postinstall (see package.json)
    ...
    // Initialize adult metadata providers from settings
    const adultMetaSettings = settings.adultMetadata;
    if (adultMetaSettings.tpdb.enabled) {
      metadataAggregator.registerProvider(AdultMetadataSource.TPDB, {
        enabled: true,
        apiKey: adultMetaSettings.tpdb.apiKey,
        priority: adultMetaSettings.tpdb.priority,
      });
    }
    if (adultMetaSettings.r18.enabled) {
      metadataAggregator.registerProvider(AdultMetadataSource.R18, {
        enabled: true,
        apiKey: adultMetaSettings.r18.apiKey,
        priority: adultMetaSettings.r18.priority,
      });
    }
    if (adultMetaSettings.adultdvdempire.enabled) {
      metadataAggregator.registerProvider(AdultMetadataSource.ADULT_DVD_EMPIRE, {
        enabled: true,
        apiKey: adultMetaSettings.adultdvdempire.apiKey,
        priority: adultMetaSettings.adultdvdempire.priority,
      });
    }
    if (adultMetaSettings.nhentai.enabled) {
      metadataAggregator.registerProvider(AdultMetadataSource.NHENTAI, {
        enabled: true,
        priority: adultMetaSettings.nhentai.priority,
      });
    }
    if (adultMetaSettings.hanime.enabled) {
      metadataAggregator.registerProvider(AdultMetadataSource.HANIME, {
        enabled: true,
        priority: adultMetaSettings.hanime.priority,
      });
    }
    if (adultMetaSettings.fakku.enabled) {
      metadataAggregator.registerProvider(AdultMetadataSource.FAKKU, {
        enabled: true,
        priority: adultMetaSettings.fakku.priority,
      });
    }
    ...
    // Health check endpoints (no auth, root level for K8s probes)
    ...
    const httpServer = host
      ? server.listen(port, host, () => {
          logger.info(`Server ready on ${host} port ${port}`, { label: 'Server' });
        })
      : server.listen(port, () => {
    ...
    // Phase 14: Graceful shutdown on SIGTERM/SIGINT
    const shutdown = async (signal: string) => {
      logger.info(`Received ${signal}, draining connections...`, { label: 'Server' });
      // Stop accepting new connections
      httpServer.close(() => logger.info('HTTP server closed', { label: 'Server' }));
